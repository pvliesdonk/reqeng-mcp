...
| `intent.py` | **NEW (Task 14)** | `validate_intent` helper (length floor, placeholder rejection); reused by Phase 2 write tools |
| `icons.py` | **NEW (Task 17)** | `TOOL_ICONS` Lucide-icon mapping for `register_tool_icons` (spec §6.9) |
...
| `tools.py` | **MODIFY** | Replace `ping` with all Phase 1 read + lifecycle + validation + export tools; `_PROJECT_ID_RE` (spec §6.4) |
...
| `server.py` | **MODIFY** | Update `domain_line` (§8.4); wire `register_server_info_tool` upstream (§5.7) and `register_tool_icons` (§6.9) |
...
| `docs/tools/index.md` | **MODIFY** | Tool list + icon hints + server-info |
...
In `pyproject.toml`, the `fastmcp-pvl-core` pin is already at `>=2.0.0,<3` (set by the copier-update v1.5.1 + spec §10.3). Between the `PROJECT-DEPS-START` and `PROJECT-DEPS-END` sentinels add only the domain-specific dependency:

```toml
dependencies = [
    "fastmcp-pvl-core>=2.0.0,<3",  # already present; verify, do not downgrade
    "typer>=0.12",
...
The contract test in Task 19 (`test_default_grammar_drift`) asserts the bundled and fixture copies match — drift triggers a deliberate pin update.
...
class Substrate:
    ...
    def __init__(
        ...
        # Public — Phase 2's write_pipeline reads these as `self.autocommit` /
        # `self.git_push_delay_s`. Read-only after construction; no underscore
        # because they're part of the published Substrate surface.
        self.autocommit = autocommit
        self.git_push_delay_s = git_push_delay_s
...
def get_substrate(ctx: Context = CurrentContext()) -> Substrate:  # noqa: B008
    """Dependency-injection accessor for tools.

    FastMCP's Depends() resolves this from the lifespan's yielded dict.
    Verified shape (fastmcp 2.x):
    - Context lives at fastmcp.Context (re-exported from fastmcp.server.context).
    - Lifespan-yielded dict is exposed as ctx.lifespan_context (NOT
      .lifespan_state — that name does not exist).
    - CurrentContext() (from fastmcp.dependencies) is the canonical DI
      injector for Context inside Depends-resolved factories.
    """
    return ctx.lifespan_context["substrate"]
```

Add the imports at the top of `_server_deps.py` (or wherever `get_substrate` lives in this project):
...
def _register_validation(mcp: FastMCP) -> None:
    ...
    async def validate_project(
        ...
        Returns ``{"findings": [...]}`` — same shape as ``check_integrity``.
        Each finding carries ``check``, ``severity``, ``message``, ``location``.
        """
        resolved = _require_project(substrate, project_id)
        backend = substrate.get_backend(resolved)
        return {
    ...
    async def check_integrity(
        ...
        Reserved seam for project-defined invariants in Phase 2 (see
        `register_integrity_check`). Returns the same shape as
...
**Files:**
- Create: `src/reqeng_mcp/intent.py` (new module — `validate_intent` helper, reused by Phase 2 write tools)
...
`create_project` also enforces the project-id naming constraint (spec §6.4): `^[a-z0-9][a-z0-9_-]{0,62}$`. The constraint is forced by the ACL scope vocabulary (`<project_id>:<verb>`, spec §7.4) — even though Phase 1 doesn't ship ACL, project IDs created in Phase 1 must remain valid when Phase 2 turns ACL on, so the validation lands now.
...
- [ ] **Step 5: Append `_register_lifecycle` to tools.py — using the helpers**

Add the project-id regex constant at the top of `src/reqeng_mcp/tools.py` (just below the existing imports):

```python
import re

# Project IDs participate in the ACL scope vocabulary as <project_id>:<verb>
# (spec §6.4, §7.4). Enforce at creation so Phase 2's ACL never sees a
# malformed project_id.
_PROJECT_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,62}$")
```

Then append `_register_lifecycle`:
...
def _register_lifecycle(mcp: FastMCP) -> None:
    ...
    async def create_project(
        ...
        `project_id` must match `^[a-z0-9][a-z0-9_-]{0,62}$` — see spec §6.4.
        """
        from reqeng_mcp.git_strategy import GitWriteStrategy
        ...
        if not _PROJECT_ID_RE.fullmatch(project_id):
            raise ValueError(
                f"project_id {project_id!r} must match {_PROJECT_ID_RE.pattern} "
                "— project IDs are encoded into ACL scope strings as "
                "<project_id>:<verb> (spec §6.4, §7.4)"
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "spec a",             # whitespace
        "spec.a",             # dot
        "spec:a",             # colon — would corrupt scope-string parse
        "spec*",              # asterisk — would collide with super-admin grant
        "_leading-underscore",
        "-leading-dash",
        "",                   # empty
        "a" * 64,             # too long (>63)
...
async def test_create_project_rejects_malformed_project_id(
    ...
    with pytest.raises(Exception) as exc:
        await mcp_client.call_tool(
            ...
            {
                "project_id": bad_id,
                "intent": "Bootstrap a project to verify the naming check.",
            },
        )
    msg = str(exc.value).lower()
    assert "project_id" in msg or "match" in msg
...
Expected: 4 base tests pass plus the parameterised `test_create_project_rejects_malformed_project_id` (9 parameter cases) — 13 tests total.
...
- create_project: git init, strictdoc_config.py, grammar materialise,
  ...
  remote_url given
- create_project: enforces project-id regex
  ^[a-z0-9][a-z0-9_-]{0,62}\$ — IDs are encoded into ACL scope strings
  as <project_id>:<verb> (spec §6.4, §7.4)
...
Refs design spec §6.3, §6.4, §7.4, §8.2"
```
...
> **Note:** Spec §6.5 specifies exports return a `file_ref` via the existing `register_file_exchange` middleware (already wired in `server.py` from the copier scaffold). For Phase 1 simplicity this plan returns a server-local path dict instead — the path-vs-file_ref transition lands as a Phase 2 polish step alongside the write tool surface, when the file-exchange runtime handle is threaded onto the substrate. The spec's Revisions block records this as a deliberate Phase 1 deviation, not a permanent decision.
...
def register_resources(mcp: FastMCP) -> None:
    ...
    @mcp.resource("spec://{project}/documents")
    async def documents_index_resource(
        project: str,
        substrate: Substrate = Depends(get_substrate),
    ) -> str:
        """Index of .sdoc documents in the project (spec §6.6)."""
        import json
        substrate.resolve_project_id(project)
        backend = substrate.get_backend(project)
        return json.dumps(
            [
                {"document": d.path.name, "title": d.title}
                for d in backend.list_documents()
            ]
        )
...
@pytest.mark.asyncio
async def test_documents_index_resource(
    mcp_client, minimal_project, monkeypatch
) -> None:
    monkeypatch.setenv("REQENG_MCP_SPEC_ROOT", str(minimal_project))
    res = await mcp_client.read_resource("spec://minimal/documents")
    payload = res.contents[0].text if hasattr(res, "contents") else res
    import json as _json
    docs = _json.loads(payload)
    assert any(d["document"] == "calculator.sdoc" for d in docs)
...
Expected: all 5 tests pass (one per resource URI in spec §6.6: projects, documents-index, document, grammar, node).
...
- spec://{project}/documents (index), spec://{project}/documents/{doc}, spec://{project}/nodes/{uid}
...
## Task 17: Server integration — domain_line, server-info upstream, tool icons

**Files:**
- Modify: `src/reqeng_mcp/server.py`
- Create: `src/reqeng_mcp/icons.py` (icon-spec mapping for `register_tool_icons`)
- Create: `tests/unit/test_icons.py`

Three small wiring steps in `make_server()`:

1. Update the `domain_line` (~80 words per spec §8.4) to name the authoring-loop steps and the Phase 3 prompts.
2. Wire `register_server_info_tool` with `upstream_label="strictdoc"` and `upstream_version=lambda: importlib.metadata.version("strictdoc")` so the LLM can see the wrapped library version through `get_server_info` (spec §5.7).
3. Wire `register_tool_icons` with the Lucide icon mapping from spec §6.9.

- [ ] **Step 1: Update build_instructions call in server.py**

Replace the `build_instructions(...)` call in `server.py` (the call near the `FastMCP(...)` constructor; line numbers drift across copier-template versions, so locate it by the `build_instructions(` string rather than a literal line):
...
- [ ] **Step 2: Wire upstream-version reporting on register_server_info_tool**

The copier-update v1.5.1 scaffold already calls `register_server_info_tool(...)` in `server.py` between the `DOMAIN-UPSTREAM-START`/`DOMAIN-UPSTREAM-END` sentinels. Replace the sentinel block contents with the StrictDoc wiring (spec §5.7):
...
register_server_info_tool(
    ...
    # DOMAIN-UPSTREAM-START — strictdoc is reqeng-mcp's wrapped library; expose
    # its installed version through get_server_info per spec §5.7.
...
`_pkg_version` is already imported from `importlib.metadata` at the top of the scaffolded `server.py`.

- [ ] **Step 3: Drop placeholder Lucide SVGs into `src/reqeng_mcp/static/icons/`**

The directory already exists from the copier-update v1.3.0 scaffold (with `.gitkeep`). `register_tool_icons` reads each filename from disk via `make_icon` and emits a base64 data URI; the file extension must be `.svg`/`.png`/`.ico`/`.jpg`/`.jpeg`. For Phase 1 we ship six small SVG placeholders covering the categories — replace later with hand-picked Lucide icons:

```bash
cd /mnt/code/reqeng-mcp/src/reqeng_mcp/static/icons/
# Each placeholder is a 16x16 SVG with a category label. Keep them tiny;
# clients render their own visualisation if available.
for name in clipboard-list file-text search git-graph history shield-check download folder-plus archive pencil trash-2 shield-user; do
    cat > "$name.svg" <<EOF
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16">
  <text x="0" y="12" font-size="6" font-family="monospace">$name</text>
</svg>
EOF
done
```

(Once the Phase 1 PR is reviewed, swap these placeholder SVGs for the actual Lucide icons via a follow-up commit; the wiring stays the same.)

- [ ] **Step 4: Create the icon-name mapping module**

Create `src/reqeng_mcp/icons.py`:

```python
"""Lucide icon hints for reqeng-mcp tools (spec §6.9).

Wired via fastmcp_pvl_core.register_tool_icons in server.py. The
register_tool_icons signature is

    register_tool_icons(mcp, mapping: Mapping[str, IconSpec], *, static_dir)

where IconSpec is `str | Path | Sequence[str | Path]` — values are
filenames relative to `static_dir`, NOT pre-built Icon objects. The
helper calls make_icon under the hood. Keep the mapping flat strings.

NOTE: register_tool_icons validates the mapping eagerly — any tool name
that isn't registered on the FastMCP instance at call time raises
ValueError. Phase 1 ships ONLY Phase 1 tool names below; Phase 2 adds
its tool entries to TOOL_ICONS in the same commit that registers each
write/ACL tool (see Phase 2 plan Tasks 3-5 and 9). Doing so keeps
TOOL_ICONS the single source of truth without crashing the Phase 1
server at startup.
"""

from __future__ import annotations

# Phase 1 tool entries only — Phase 2 plan Tasks 3-5 (writes) and Task 9
# (ACL admin) extend this mapping in lockstep with the tools they
# register.
TOOL_ICONS: dict[str, str] = {
    # List / browse
    "list_projects":       "clipboard-list.svg",
    "list_documents":      "clipboard-list.svg",
    "list_nodes":          "clipboard-list.svg",
    "list_relations":      "clipboard-list.svg",
    # Get / read
    "get_grammar":         "file-text.svg",
    "get_project_status":  "file-text.svg",
    "get_document":        "file-text.svg",
    "get_node":            "file-text.svg",
    "get_field":           "file-text.svg",
    # Search
    "search_nodes":        "search.svg",
    # Trace / graph
    "traceability_matrix": "git-graph.svg",
    "dependents_of":       "git-graph.svg",
    "dependencies_of":     "git-graph.svg",
    # History / diff
    "get_node_history":    "history.svg",
    "get_node_diff":       "history.svg",
    # Validation / integrity
    "validate_project":    "shield-check.svg",
    "check_integrity":     "shield-check.svg",
    # Export
    "export_html":         "download.svg",
    "export_reqif":        "download.svg",
    "export_excel":        "download.svg",
    "export_markdown":     "download.svg",
    # Lifecycle
    "create_project":      "folder-plus.svg",
    "archive_project":     "archive.svg",
}
```

- [ ] **Step 5: Wire register_tool_icons in server.py**

Imports go at the **module top** of `server.py` (ruff I001 enforces this):
...
from reqeng_mcp.icons import TOOL_ICONS
```

Then in the `make_server()` body, after the `register_server_info_tool` call:
...
(Reordering vs. an inline `import` keeps `uv run ruff check` clean.)

- [ ] **Step 6: Write a sanity test for the icon mapping**

Create `tests/unit/test_icons.py`:

```python
"""Sanity checks on the icon-name table (spec §6.9)."""
from __future__ import annotations
...
_STATIC_DIR = Path(__file__).resolve().parents[2] / "src" / "reqeng_mcp" / "static" / "icons"


def test_icon_table_covers_phase1_tools() -> None:
    phase1_tools = {
        "list_projects", "list_documents", "list_nodes", "list_relations",
        "get_grammar", "get_project_status", "get_document", "get_node",
        "get_field", "search_nodes", "traceability_matrix", "dependents_of",
        "dependencies_of", "get_node_history", "get_node_diff",
        "validate_project", "check_integrity",
        "export_html", "export_reqif", "export_excel", "export_markdown",
        "create_project", "archive_project",
    }
    assert phase1_tools.issubset(TOOL_ICONS.keys())


def test_no_phase2_tools_in_phase1_table() -> None:
    # register_tool_icons validates eagerly — Phase 2 tool names in
    # the Phase 1 mapping would crash server start. Phase 2 plan Tasks
    # 3-5 and 9 add their entries when those tools are registered.
    forbidden_in_phase1 = {
        "create_node", "set_fields", "add_relation", "remove_relation",
        "move_node", "rename_uid", "delete_node",
        "acl_list_subjects", "acl_grant", "acl_revoke",
        "acl_grant_global", "acl_revoke_global",
    }
    assert forbidden_in_phase1.isdisjoint(TOOL_ICONS.keys())


def test_every_referenced_icon_file_exists() -> None:
    # register_tool_icons -> make_icon -> file_path.read_bytes() — missing
    # files raise FileNotFoundError at server start. Catch that here.
    for tool, filename in TOOL_ICONS.items():
        assert (_STATIC_DIR / filename).is_file(), (
            f"icon file {filename!r} for tool {tool!r} missing from "
            f"{_STATIC_DIR}"
        )


def test_no_duplicate_keys() -> None:
    # dict literal would silently keep the last duplicate; assert via length.
    raw_count = sum(1 for _ in TOOL_ICONS)
    assert raw_count == len(TOOL_ICONS)
```

- [ ] **Step 7: Run the tests**

```bash
uv run pytest tests/unit/test_icons.py tests/test_smoke.py -v
```

Expected: green.

- [ ] **Step 8: Verify all integration tests still pass**
...
```bash
git add src/reqeng_mcp/server.py src/reqeng_mcp/icons.py src/reqeng_mcp/static/icons/*.svg tests/unit/test_icons.py
git commit -m "feat(server): domain_line, server-info upstream, tool icons

- domain_line: one-paragraph baseline per spec §8.4 (~80 words);
  names 6 loop steps + 4 Phase-3 prompt names; read_only=False
- register_server_info_tool: wire upstream_label='strictdoc' +
  upstream_version reading installed strictdoc version (spec §5.7)
- register_tool_icons: Lucide icon hints for Phase 1 tools (spec §6.9);
  Phase 2 tasks extend TOOL_ICONS in lockstep with each tool registration
  in src/reqeng_mcp/icons.py

Refs design spec §5.7, §6.9, §8.4"
```
...
@pytest.mark.asyncio
async def test_full_loop_read_only(
    mcp_client, minimal_project, monkeypatch
) -> None:
    monkeypatch.setenv("REQENG_MCP_SPEC_ROOT", str(minimal_project))
    ...
    # check_integrity returns {"findings": [...]} (spec §6.5).
    errors = [x for x in fnd["findings"] if x["severity"] == "error"]
    assert errors == []
...
def test_default_grammar_drift() -> None:
    """The bundled default.sgra and the fixture copy must stay byte-identical.

    Per Task 1 Step 9, ``tests/fixtures/grammars/default.sgra`` is pinned
    against ``src/reqeng_mcp/grammars/default.sgra``. Drift means an
    accidental change to the bundled grammar leaked through; the failure
    is the prompt to deliberately update both files together (and bump
    StrictDoc-pin discussion in the spec if relevant).
    """
    from pathlib import Path

    repo = Path(__file__).resolve().parents[2]
    bundled = (repo / "src" / "reqeng_mcp" / "grammars" / "default.sgra").read_bytes()
    fixture = (repo / "tests" / "fixtures" / "grammars" / "default.sgra").read_bytes()
    assert bundled == fixture, (
        "bundled default.sgra drifted from the test fixture copy — update both "
        "files in lockstep (Phase 1 plan Task 1 Step 9)"
    )
...
| `REQENG_MCP_ACL_PATH` | unset | Reserved for Phase 2 (path to `.reqeng-acl.toml`; absence = no authorization) |
...
Built into the template via the `[debug]` extra (spec §7.6). All deployment modes support an optional debugpy listener:

| Env var | Default | Purpose |
|---|---|---|
| `REQENG_MCP_DEBUG_PORT` | unset | When set (e.g. `5678`), `maybe_start_debugpy` opens a debugpy listener at server start. Requires the `[debug]` extra to be installed. |
| `REQENG_MCP_DEBUG_WAIT` | `0` | When `1`/`true`, the listener blocks startup until a debugger attaches. |

Install the extra locally with `uv sync --extra debug`. The Docker image accepts `--build-arg DEBUG=true` to bake debugpy into the production image. Off by default — never wired in production deployments.
````
...
Each tool carries a Lucide icon hint via `register_tool_icons` (spec §6.9).
Clients that surface icons in their tool picker render them (e.g. `clipboard-list`
for `list_*`, `git-graph` for `traceability_matrix`, `download` for `export_*`,
`shield-check` for validation). Clients that don't surface icons silently ignore
the metadata. The mapping lives in `src/reqeng_mcp/icons.py`.

## Server info

The standard `get_server_info` tool reports both reqeng-mcp's own version and
the installed StrictDoc version (spec §5.7). Useful when triaging parse-error
reports — confirms which StrictDoc version is actually loaded.
````
...
**Scope check:** plan covers Phase 1 only, per spec §10.1. Phase 2 (write substrate + ACL) plan lands separately on its own branch — no remaining upstream blockers (pvl-core 2.0 ships the full authz surface, and `fastmcp-server-template` v1.5.0+ ships the scaffolded stubs already present in this repo). Phase 3 (authoring channels + worked synthetic) gets its own plan after Phase 2 lands.
