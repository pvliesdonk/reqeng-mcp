...
|------|-------------|---------------|
| **Multi-auth** | Mixed clients — e.g. Claude web (OIDC) + Claude Code (bearer token) on the same server | Set both `REQENG_MCP_BEARER_TOKEN` and all four OIDC variables |
...
The bearer-token mode above shares one subject across every authenticated caller — by default the library's `bearer-anon`, override with `REQENG_MCP_BEARER_DEFAULT_SUBJECT`.  For audit logs and authorization that distinguish callers, switch to mapped-token mode by pointing `REQENG_MCP_BEARER_TOKENS_FILE` at a TOML file:
...
|----------|-------------|
| `REQENG_MCP_BASE_URL` | Public base URL (e.g. `https://mcp.example.com`) |
...
| `REQENG_MCP_OIDC_AUDIENCE` | — | Expected JWT audience claim; leave unset if your provider does not set one |
